...
def get_weight_pr(W):
    """
    Compute the Weight Participation Ratio for each hidden neuron's incoming weight vector.
    PR(w_j) = (sum_i |W_ij|)^2 / sum_i (W_ij^2)
    High PR means the neuron is highly superposed (polysemantic).
    """
    l1_norm = W.abs().sum(dim=0)
    l2_sq_norm = (W ** 2).sum(dim=0)
    # Add epsilon to prevent division by zero
    pr_w = (l1_norm ** 2) / (l2_sq_norm + 1e-8)
    return pr_w

def split_polysemantic_neurons(W, n_children=2, reference_W=None, target_fraction=1.0):
    ...
    # Sort neurons by superposition (Weight PR) if target_fraction < 1.0
    if target_fraction < 1.0:
        pr_w = get_weight_pr(reference_W)
        n_target = max(1, int(m * target_fraction))
        _, sorted_indices = torch.sort(pr_w, descending=True)
        target_indices = set(sorted_indices[:n_target].tolist())
    else:
        target_indices = set(range(m))

    for j in range(m):
        w = W[:, j]
        ref_w = reference_W[:, j]
        
        # Only split if it's in our target set
...
def main():
    ...
    # Dynamic Trigger / Targeted FPE args
    parser.add_argument("--patience", type=int, default=5, help="Number of logging intervals to wait for plateau.")
    parser.add_argument("--tolerance", type=float, default=0.5, help="D_PR plateau threshold.")
    parser.add_argument("--target_fraction", type=float, default=1.0, help="Fraction of most superposed neurons to expand.")
    ...
    losses = []
    pr_dims = []
    recent_pr_dims = [] # Track recent logs for dynamic plateau trigger

    for step in range(args.n_steps_pre):
        x = (torch.rand(args.batch_size, args.n, device=device) < prob) * torch.rand(args.batch_size, args.n, device=device) * 2
        lr = get_lr(step, 1e-2, args.n_steps_pre)
        ...
        if (step+1) % args.log_interval == 0:
            print(f"step {step+1}/{args.n_steps_pre} | Phase 1 Loss: {loss.item():.4e} D_PR: {pr_dim:.2f}")
            recent_pr_dims.append(pr_dim)
            
            # Dynamic trigger check
            if len(recent_pr_dims) >= args.patience:
                window = recent_pr_dims[-args.patience:]
                dpr_diff = max(window) - min(window)
                if dpr_diff < args.tolerance:
                    print(f"\\n--> Dynamic Trigger! D_PR has plateaued (diff {dpr_diff:.3f} < {args.tolerance:.3f} over last {args.patience} logs). Breaking early at step {step+1}.")
                    split_step = step + 1
                    n_steps_total = split_step + args.n_steps_post
                    break

    W_old = model.W.detach()
    W_new, split_indices, expanded_indices = split_polysemantic_neurons(W_old, target_fraction=args.target_fraction)
    m_new = W_new.shape[1]
    
    print(f"\\nTargeted FPE Expansion: m={args.m} -> m={m_new} (fraction: {args.target_fraction})")
    ...
    if len(optimizer.state) > 0:
        ...
        if 'exp_avg' in old_state_W:
            base_exp_avg, _, _ = split_polysemantic_neurons(old_state_W['exp_avg'], reference_W=W_old, target_fraction=args.target_fraction)
            base_exp_avg_sq, _, _ = split_polysemantic_neurons(old_state_W['exp_avg_sq'], reference_W=W_old, target_fraction=args.target_fraction)
            for opt, mod in [(optimizer_full, model_full), (optimizer_quant, model_quant)]:
