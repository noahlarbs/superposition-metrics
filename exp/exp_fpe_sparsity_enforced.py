...
def main():
    ...
    # Keep a completely separate baseline model that does NOT get split
    import copy
    ...
    # Initialize baseline baseline_optimizer state to continue smoothly
    baseline_optimizer.zero_grad()
    dummy_loss_b = baseline_model(torch.zeros(1, args.n, device=device)).sum()
    dummy_loss_b.backward()
    baseline_optimizer.step()
    
    # Copy exactly the same optimizer states over
    if len(optimizer.state) > 0:
        if 'exp_avg' in optimizer.state[model.W]:
            baseline_optimizer.state[baseline_model.W]['exp_avg'] = optimizer.state[model.W]['exp_avg'].clone()
            baseline_optimizer.state[baseline_model.W]['exp_avg_sq'] = optimizer.state[model.W]['exp_avg_sq'].clone()
            baseline_optimizer.state[baseline_model.W]['step'] = optimizer.state[model.W]['step']
            
            baseline_optimizer.state[baseline_model.b]['exp_avg'] = optimizer.state[model.b]['exp_avg'].clone()
            baseline_optimizer.state[baseline_model.b]['exp_avg_sq'] = optimizer.state[model.b]['exp_avg_sq'].clone()
            baseline_optimizer.state[baseline_model.b]['step'] = optimizer.state[model.b]['step']

    # Safely overwrite the old model and optimizer to be the new FPE ones
    model = new_model
    optimizer = new_optimizer

    losses_baseline = list(losses)
    pr_dims_baseline = list(pr_dims)
    ...
    print(f"\n--- Phase 2: Post-FPE training & Baseline Validation (steps {split_step+1}–{n_steps_total}) ---")
    ...
    for step in range(split_step, n_steps_total):
        ...
        if (step + 1) % args.log_interval == 0:
            print(
                f"  step {step + 1}/{n_steps_total} | FPE loss: {loss.item():.4e} D_PR: {pr_dim:.2f} | Base loss: {loss_b.item():.4e} D_PR: {pr_dim_b:.2f}"
            )
    ...
    e = delta_log["end"]
    print(f"  Before FPE:     loss={b['loss']:.4e}, D_PR={b['pr_dim']:.2f}")
    print(f"  After FPE:      loss={a['loss']:.4e}, D_PR={a['pr_dim']:.2f}  (Δloss={a['loss']-b['loss']:+.4e}, ΔD_PR={a['pr_dim']-b['pr_dim']:+.2f})")
    print(f"  End FPE Model:  loss={e['loss']:.4e}, D_PR={e['pr_dim']:.2f}  (Δloss={e['loss']-a['loss']:+.4e}, ΔD_PR={e['pr_dim']-a['pr_dim']:+.2f})")
    print(f"  End Base Model: loss={losses_baseline[-1]:.4e}, D_PR={pr_dims_baseline[-1]:.2f}")
