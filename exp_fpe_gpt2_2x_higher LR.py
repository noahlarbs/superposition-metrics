...
def run_experiment(args, device, train_loader, val_loader, tokenizer, vocab_size, run_mode):
    ...
    for step in range(args.n_steps_max):
        ...
        if (step + 1) % args.log_interval == 0 or step == 0:
            ...
            # --- NEW SAFETY LOGGER ---
            with open(f"backup_log_{run_mode}.txt", "a") as log_file:
                log_file.write(f"{step+1},{active_wall_clock:.1f},{val_ppl:.2f},{pr_dim:.2f}\n")
            
            # FPE Trigger Logic
            # Changed to allow custom run names (e.g., fpe_tol_0.5)
            if not has_expanded and "fpe" in run_mode and len(dpr_stats) >= args.patience:
...
def main():
    ...
    # Testing different tolerances. A tighter (lower) tolerance requires a flatter D_PR plateau,
    # which forces the model to stay in the smaller geometry longer (detonating later).
    tolerances_to_test = [0.25, 0.1]
    results = {}

    for tol in tolerances_to_test:
        args.tolerance = tol
        run_name = f"fpe_tol_{tol}"
        eval_stats, dpr_stats = run_experiment(args, device, train_loader, val_loader, tokenizer, vocab_size, run_name)
        results[run_name] = {"eval": eval_stats, "dpr": dpr_stats}
    ...
    for i, (run_name, data) in enumerate(results.items()):
        color = colors[i % len(colors)]
        eval_data, dpr_data = data["eval"], data["dpr"]

        # ax1: PPL vs Wall Clock
        ax1.plot([e[0] for e in eval_data], smooth([e[1] for e in eval_data]), label=run_name, color=color, linewidth=3, alpha=0.9)

        # ax2: DPR vs Steps
        ax2.plot([d[0] for d in dpr_data], smooth([d[1] for d in dpr_data]), label=run_name, color=color, linewidth=3, alpha=0.9)
