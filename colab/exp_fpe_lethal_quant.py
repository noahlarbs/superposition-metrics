%%writefile exp_fpe_lethal_quant.py
"""
Targeted FPE Transformer on TinyStories - Lethal Quantization Threshold Hunter

This script executes a Width Ablation study over highly constrained FeedForward 
dimensions (e.g., d_ff = 32, 64, 128). It deliberately starves the network of parameters 
to force immense natural superposition.

Upon finding the geometric D_PR plateau, it drops a "Lethal Quantization Hammer", applying 
BitNet 1.58b [-1, 0, 1] quantization to 100% of the active FFN neurons. 

By testing the strict mathematical Control (direct 100% quantization) against the 
Fixed Parameter Expansion intervention (expanding 100% to double orthogonal space), 
we establish the interpolation threshold where overparameterization stops protecting 
the model, and prove FPE rescues the collapse.
"""
...
def run_experiment(args, device, dataloader, tokenizer, vocab_size, d_ff_current, run_mode):
    ...
    print(f"\n{'='*50}")
    print(f"Starting Run: d_ff={d_ff_current} | mode={run_mode.upper()} | fraction={args.target_fraction}")
    print(f"{'='*50}")
    ...
    print(f"\n--- Phase 2: LETHAL Quantization Intervention ({run_mode.upper()}) ---")
...
def main():
    ...
    parser.add_argument("--d_ff_list", type=int, nargs="+", default=[32, 64, 128, 256])
    ...
    parser.add_argument("--target_fraction", type=float, default=1.0) # Lethal Quantization
    ...
    print("\nStarting Threshold Hunter Ablation...")
    
    for d_ff in args.d_ff_list:
        results[d_ff] = {}
        
        # Always run FPE
        losses_fpe, pr_dims_fpe, trigger_step_fpe = run_experiment(args, device, dataloader, tokenizer, vocab_size, d_ff, "fpe")
        results[d_ff]["fpe_losses"] = losses_fpe
        results[d_ff]["fpe_prs"] = pr_dims_fpe
        results[d_ff]["fpe_trigger"] = trigger_step_fpe
        
        # Test constraints to find interpolation threshold limit
        if d_ff in [32, 64, 128]:
            losses_ctrl, pr_dims_ctrl, trigger_step_ctrl = run_experiment(args, device, dataloader, tokenizer, vocab_size, d_ff, "control")
            results[d_ff]["ctrl_losses"] = losses_ctrl
            results[d_ff]["ctrl_prs"] = pr_dims_ctrl
            results[d_ff]["ctrl_trigger"] = trigger_step_ctrl
    ...
    # Plot 1: Multi-panel Threshold Comparison (FPE vs Control for [32, 64, 128])
    fig, axes = plt.subplots(1, 3, figsize=(18, 5))
    fig.suptitle("Interpolation Threshold Hunter: Lethal 100% Quantization (FPE vs Control Baseline)", fontsize=16)
    
    for idx, test_dff in enumerate([32, 64, 128]):
        ax = axes[idx]
        if test_dff in results and "ctrl_losses" in results[test_dff]:
            fpe_l = results[test_dff]["fpe_losses"]
            ctrl_l = results[test_dff]["ctrl_losses"]
            trig_fpe = results[test_dff]["fpe_trigger"]
            
            ax.plot(range(len(fpe_l)), fpe_l, label="FPE (Doubled Space)", color="blue", alpha=0.8)
            ax.plot(range(len(ctrl_l)), ctrl_l, label="Strict Control (Static)", color="red", alpha=0.8)
            ax.axvline(x=trig_fpe, color='black', linestyle='--', label=f'Trigger ({trig_fpe})')
            
            ax.set_title(f"Constrained Bottleneck: d_ff={test_dff}")
            ax.set_xlabel("Training Steps")
            ax.set_ylabel("Loss")
            ax.legend()
            ax.grid(True, alpha=0.3)
            ax.set_ylim(top=min(max(fpe_l[trig_fpe:] + ctrl_l[trig_fpe:]) * 1.5, 10.0))
            
    plt.tight_layout()
    plt.savefig("../newfigures/loss_vs_step_thresholds.png", dpi=300, bbox_inches="tight")
    print("\nSaved Threshold Grid Figure: ../newfigures/loss_vs_step_thresholds.png")
